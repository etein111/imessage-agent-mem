...
class RedisMemoryStore:
    ...
    def add_message(self, user_id, role, content):
        ...
        self.client.rpush(self._get_chat_key(user_id), json.dumps(msg))
        self.client.expire(self._get_chat_key(user_id), self.ttl)

    def get_context(self, user_id, limit=None) -> List[Dict]:
        """
        获取对话上下文。
    ...
    def get_summary(self, user_id) -> str:
        return self.client.get(self._get_summary_key(user_id)) or ""

    def update_summary(self, user_id, summary):
        self.client.set(self._get_summary_key(user_id), summary)

    # --- 溢出处理 ---
    def check_and_extract_overflow(self, user_id) -> List[Dict]:
        """
        ...
        if current_len >= self.trigger_limit:
            ...
            popped_raw = self.client.lrange(key, 0, self.batch_size - 1)

            if popped_raw:
                self.client.ltrim(key, self.batch_size, -1)
